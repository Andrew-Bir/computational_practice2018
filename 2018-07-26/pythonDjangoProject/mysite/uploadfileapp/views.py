...
class DetectItem(TemplateView):
    ...
    def post(self, request, pk):
        ...
        if form.is_valid():
            t_dir = settings.BASE_DIR + form.cleaned_data['path'] + pk
            fname = settings.BASE_DIR + file_name
            parent_obj = pk;
            dir = detect_and_crop(fname, t_dir, parent_obj)

        args = {'form': form, 'title': title, 'file_name': file_name, 'dir': dir}
        return render(request, self.template_name, args)


def detect_and_crop(image_path, target_dir, parent_obj):
    # Read the image
    ...
    msg = cropFunction(image, faces, target_dir, parent_obj)
...
def cropFunction(image, faces, directory, parent_obj):
    i = 0
    ...
    for (x, y, w, h) in faces:
        ...
        # model FileField
        a_record.file = "results/"+parent_obj+"/image" + str(i) + ".jpg"
